...
For $X \subset \R^n$, vectorfield $f: X \to \R^n, g: X \to \R \in C^1$ s.t. $\bigtriangledown g = f$ and parametrized curve $\gamma: [a, b] \to \R^n, \gamma([a, b]) \subset X$ then: $\int_{\gamma} f \mathrm{d}t = \int_{a}^{b} f(\gamma(t)) \cdot \gamma'(t) \mathrm{d}t = \int_{a}^{b} \bigtriangledown g(\gamma(t)) \cdot \gamma'(t) \mathrm{d}t = \int_{a}^{b} \frac{d}{dt} (g \circ \gamma) \mathrm{d}t = (g \circ \gamma)(b) - (g \circ \gamma)(a)$. I.e. the integral only depends on the endpoints of the curve.

\subsubsection{Potential}
For $X \subset \R^n$, scalar field $g: X \to \R$ and $f: X \to \R^n$, where $\bigtriangledown g = f$, then $g$ is a potential for $f$.

\begin{compactitem}
    \item Does always exists for $n = 1$ and is equivalent to the primitive (Ausleitung) of $f$
\end{compactitem}
...
\begin{compactitem}
    ...
    \item if $f$ is conservative, then $\partial_j f_i = \partial_i f_j \quad \forall 1 \le i \neq j \le n$.
        \begin{compactitem}
            \item if $f$ is in addition star-shapend, this condition holds in both directions.
        \end{compactitem}
\end{compactitem}
...
\begin{compactitem}
    ...
    \item Frobenius: If $Q = I_1 \times \dots \times I_n$ and $f$ continuous on $Q$ then $\int_{Q} f(x_1, \dots x_n) \mathrm{d}x_1,\dots,x_n = \int_{a_1}^{b_1} \int_{a_2}^{b_2} \dots \int_{a_n}^{b_n} f(x_n) \mathrm{d}x_n \dots \mathrm{d}x_2 \mathrm{d}x_1$.
...
\begin{compactitem}
    ...
    \item Given a two dimensional integral $\int\int_X g(x, y) \mathrm{d}x\mathrm{y}$ which we want to evaluate. If we can find $f = (f_1, f_2)$ with $\text{curl }f = g$ then we can use $\int\int_X g \mathrm{d}x\mathrm{d}y = \int\int_X \text{curl }f \mathrm{d}x\mathrm{y} = \int_{\partial X = \gamma} f \mathfm{d} x$
\end{compactitem}
