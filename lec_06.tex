...
A \code{struct} is a C type that contains a set of fields. The tag of the structure is like the name of the structure. To referee to fields of a struct, we use the \code{tag.field} notation, to referee to fields from a pointer to the struct, we can use \code{p\_tag->field} which is the shorthand notation for \code{(*p\_tag).field}. Stucts can be initialized by either providing all values at the same time, or by assigning value by value.
