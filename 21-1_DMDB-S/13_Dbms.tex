...
\begin{itemize}
    ...
    \item Overview
        \begin{itemize}
            \ides{Query Optimization:} SQL query to relational algebra converter
            \ides{Operator Execution:} Execute a relation algebra operation
            \ides{Access Methods:} Provides different ways of accessing data from a relation
            \ides{Buffer Pool Management:} Gives illusion that all data is stored on memory
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \ides{Row Storage}
        \begin{itemize}
            ...
            \item Divided into
                \begin{itemize}
                    \ides{Bitmap:} Indicates which attributes are \verb+NULL+ (somehow)
                    \ides{Fixed-Length:}
                        \begin{itemize}
...
\begin{itemize}
    ...
    \ides{Hash Table}
        \begin{itemize}
            ...
            \ides{Hash Function $\mathbf{f(x)}$:} Maps each object into an entry in the table
                \begin{itemize}
...
\end{verbatim}
                    ...
                    \item Order of tables matters
                        ...
                        \end{itemize}
                    \ides{Efficient if:} Both relations fit into memory
                \end{itemize}
...
\end{verbatim}
            ...
            \ides{Sort Merge Join}
                \begin{itemize}
                    ...
                    \ides{Efficient if:} Index in attribute is present
                \end{itemize}
            \ides{Hash Join}
                \begin{itemize}
                    \item
...
\end{verbatim}
                    ...
                    \ides{Efficient if:} Result of join fits into memory
                \end{itemize}
    ...
    \ides{Search Algorithm:} How can we search the best physical plan, given cost model?
        \begin{itemize}
            ...
            \ides{Compromise 2:} Heuristic-based Optimisation
                \begin{itemize}
                    \item Optimize query-tree by applying a set of rules that typically improve execution performance
                        \begin{itemize}
