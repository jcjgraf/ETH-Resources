    ...
    \ides{Strict (ST):}
        \begin{itemize}
            \item If $T_i$ reads from or writes a value written by $T_j$, then
                \begin{itemize}
                    \ides{If $\mathbf{T_i}$ commits:} $(c_j < r_i[X] \wedge c_j < w_i[X])$
                    \ides{If $\mathbf{T_i}$ aborts:} $(a_j < r_i[X] \wedge a_j < w_i[X])$
                \end{itemize}
...
\begin{itemize}
    \ides{Assume:}
        \begin{itemize}
            \item Disk is save
            \item \verb+Write(A, v)+ only changes object in memory but not buffer
